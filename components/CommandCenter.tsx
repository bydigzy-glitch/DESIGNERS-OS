    ...
    return (
        <TooltipProvider delayDuration={300}>
            <div className="flex flex-col h-full w-full space-y-6 pb-24 md:pb-0 overflow-y-auto scrollbar-hide pr-2">
                ...
                {/* Two Column Layout: Handled + Approvals */}
                            ...
                            <CardContent>
                                {handledToday.length === 0 ? (
                                    <div className="text-center py-6 text-muted-foreground text-sm">
                                        No automated actions yet today
                                    </div>
                                ) : (
                                    <div className="space-y-3 max-h-[250px] overflow-y-auto scrollbar-hide">
                                        {handledToday.map(action => (
                                            <div
                                                key={action.id}
                                                className="flex items-start gap-4 p-4 rounded-xl bg-secondary/20 border border-transparent"
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="text-[10px] text-muted-foreground mt-0.5">
                                                        {action.result} • {new Date(action.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                ...
                                {pendingApprovals.length === 0 ? (
                                    ...
                                    <div className="space-y-3 max-h-[250px] overflow-y-auto scrollbar-hide">
                                        {pendingApprovals.map(approval => (
                                            <div
                                                key={approval.id}
                                                className={`p-3 rounded-lg border ${URGENCY_COLORS[approval.urgency]}`}
                                            >
                                                <div className="flex items-start justify-between gap-3">
                                                    <div className="flex-1 min-w-0">
                                                        <div className="text-sm font-medium text-foreground">{approval.title}</div>
                                                        <div className="text-xs text-muted-foreground mt-0.5">{approval.message}</div>
                                                    </div>
                                                    <div className="flex gap-1.5 flex-shrink-0">
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onReject(approval)}>
                                                                    <X size={14} />
                                                                </Button>
                                                            </TooltipTrigger>
                                                        ...
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button size="icon" className="h-7 w-7" onClick={() => onApprove(approval)}>
                                                                    <Check size={14} />
                                                                </Button>
                                                            </TooltipTrigger>
                        ...
                        <Card className="border-red-500/20">
                            <CardHeader className="pb-3">
                                <CardTitle className="text-base flex items-center gap-2 text-red-500">
                                    ...
                                    Risk Alerts
                                    <Badge variant="destructive" className="ml-2">{activeRisks.length}</Badge>
                                </CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-3">
                                    {activeRisks.map(risk => (
                                        <div
                                            key={risk.id}
                                            className={`flex items-center justify-between p-4 rounded-xl border ${SEVERITY_COLORS[risk.severity]}`}
                                        >
                                            <div className="flex items-center gap-4">
                                                <div className={`p-2 rounded-lg ${risk.severity === 'CRITICAL' ? 'bg-red-500/10 text-red-500' :
                                                    risk.severity === 'WARNING' ? 'bg-orange-500/10 text-orange-500' :
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <Badge variant="outline" className="text-[10px]">{risk.type.replace('_', ' ')}</Badge>
                                                    </div>
                                                    <div className="text-sm text-muted-foreground mt-0.5">{risk.message}</div>
                                                </div>
                                            </div>
                                            <Button variant="ghost" size="sm" onClick={() => onAcknowledgeRisk(risk)}>
                                                Acknowledge
                                            </Button>
                                        </div>
                                    ))}
