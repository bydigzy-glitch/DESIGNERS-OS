...
export const TeamPage: React.FC<TeamPageProps> = ({ 
  ...
  const handleInvite = (e: React.FormEvent) => {
      ...
      const newMember: TeamMember = {
          ...
          dailyStreak: Math.floor(Math.random() * 20) // Mock streak for demo
      };
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
      ...
      storageService.sendTeamMessage(user.id, newMsg);
      // Optimistic update handled by onUpdateUser logic in App or re-fetch in storageService
      onUpdateUser({ ...user, teamChat: [...teamChat, newMsg] });
      setChatInput('');
  };

  // Stats
  const teamTasksCompleted = tasks.filter(t => t.completed).length; 
  const combinedStreak = teamMembers.reduce((acc, m) => acc + (m.dailyStreak || 0), 0) + (user.id === 'admin-user' ? 12 : 0); // Mock offset
    ...
    <div className="flex flex-col h-full w-full space-y-6 md:space-y-8 pb-24 md:pb-0 overflow-y-auto scrollbar-hide pr-2 relative">
       ...
       <FadeIn>
          <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 pt-2 md:pt-0">
             <div>
                <h1 className="text-3xl font-bold text-foreground tracking-tight">Team HQ</h1>
          ...
          <FadeIn delay={0.2} className="md:col-span-6 lg:col-span-5 bg-card border border-border rounded-2xl p-5 shadow-sm flex flex-col justify-between group">
              ...
              <div className="flex items-center gap-4 overflow-x-auto pb-2 scrollbar-hide">
                 ...
                 <div className="flex flex-col items-center gap-1 min-w-[50px]">
                    <div className="w-12 h-12 rounded-full p-0.5 bg-gradient-to-tr from-orange-500 to-yellow-500">
                        <img src={user.avatar} className="w-full h-full rounded-full border-2 border-card object-cover" />
                    </div>
                 ...
                 {teamMembers.map(m => (
                     <div key={m.id} className="flex flex-col items-center gap-1 min-w-[50px]">
                        <div className={`w-12 h-12 rounded-full p-0.5 ${m.dailyStreak && m.dailyStreak > 0 ? 'bg-gradient-to-tr from-orange-500 to-yellow-500' : 'bg-secondary'}`}>
                            <img src={m.avatar} className="w-full h-full rounded-full border-2 border-card object-cover" />
                        </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-[600px]">
                ...
                <div className="p-4 border-b border-border flex justify-between items-center bg-secondary/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span className="text-xs text-muted-foreground">{teamMembers.length + 1} Members Online</span>
                        </div>
                    </div>
           ...
           <FadeIn delay={0.4} className="bg-card border border-border rounded-2xl flex flex-col overflow-hidden shadow-sm h-full max-h-[400px] lg:max-h-[600px] lg:h-auto">
               ...
               <div className="flex-1 overflow-y-auto p-2">
                   {/* Owner (You) */}
                   <div className="p-3 hover:bg-secondary/30 rounded-xl flex items-center justify-between group transition-colors">
                       <div className="flex items-center gap-3">
                           <div className="relative">
                               <img src={user.avatar} className="w-10 h-10 rounded-full object-cover border border-border" />
                               <span className="absolute -bottom-1 -right-1 bg-green-500 w-3 h-3 rounded-full border-2 border-card"></span>
                           </div>
                           <div>
                               <div className="font-bold text-sm text-foreground">{user.name} <span className="text-xs text-muted-foreground">(You)</span></div>
                               <div className="text-[10px] font-bold text-primary px-1.5 py-0.5 bg-primary/10 rounded w-fit mt-0.5">OWNER</div>
                           </div>
                       </div>
                   </div>
                   ...
                   {teamMembers.map(member => (
                       <div key={member.id} className="p-3 hover:bg-secondary/30 rounded-xl flex items-center justify-between group transition-colors">
                           <div className="flex items-center gap-3">
                               <div className="relative">
                                    <img src={member.avatar} className="w-10 h-10 rounded-full object-cover border border-border" />
                               ...
                               <div>
                                   <div className="font-bold text-sm text-foreground">{member.name || member.email.split('@')[0]}</div>
                                   <div className="flex items-center gap-1 mt-0.5">
                                      <button 
                                        onClick={() => updateRole(member.id, member.role === 'ADMIN' ? 'VIEWER' : 'ADMIN')}
                                        className="text-[10px] font-bold text-muted-foreground uppercase hover:text-foreground bg-secondary px-1.5 rounded cursor-pointer"
                                      >
                                          {member.role}
                                      </button>
                                   </div>
