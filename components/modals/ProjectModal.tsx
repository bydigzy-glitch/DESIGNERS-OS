      ...
      {isOpen && (
        <div className="fixed inset-0 z-[var(--z-modal)] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl relative overflow-hidden flex flex-col max-h-[65vh] md:max-h-[90vh] z-10"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-6 border-b border-border/50 flex justify-between items-center shrink-0">
              <h2 className="text-xl font-bold text-foreground">{initialProject ? 'Edit Project' : 'New Project'}</h2>
            ...
            <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
                ...
                <div>
                  ...
                  {clients.length > 0 ? (
                    <select
                      ...
                      className="w-full bg-secondary border border-border rounded-xl p-3 text-foreground focus:outline-none focus:border-primary appearance-none transition-colors"
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-secondary border border-border rounded-xl p-3 pl-8 text-foreground focus:outline-none focus:border-primary transition-colors"
                      placeholder="0.00"
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-secondary border border-border rounded-xl p-3 text-foreground focus:outline-none focus:border-primary pl-10 transition-colors"
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Link Tasks</label>
                <div className="bg-secondary/30 border border-border rounded-xl p-2 max-h-40 overflow-y-auto custom-scrollbar">
                  {availableTasks.length > 0 ? availableTasks.map(t => (
                    <div
                      key={t.id}
                      onClick={() => toggleTaskSelection(t.id)}
                      className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${selectedTaskIds.includes(t.id) ? 'bg-primary/20' : 'hover:bg-white/5'}`}
                    >
                      <div className={`w-4 h-4 rounded border flex items-center justify-center ${selectedTaskIds.includes(t.id) ? 'bg-primary border-primary' : 'border-gray-600'}`}>
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button
                      ...
                      className={`w-8 h-8 rounded-full transition-transform hover:scale-110 ${item.cls} ${color === item.hex ? 'ring-2 ring-offset-2 ring-offset-background ring-white' : ''}`}
              ...
              <div>
                <label htmlFor="project-notes" className="block text-xs font-bold text-muted-foreground uppercase mb-2">Notes</label>
                <div className="relative">
                  <textarea
                    id="project-notes"
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    className="w-full bg-secondary border border-border rounded-xl p-3 text-foreground focus:outline-none focus:border-primary pl-10 min-h-[100px] resize-none transition-colors"
            ...
            <div className="p-6 border-t border-border/50 flex justify-between items-center shrink-0">
              ...
              <button onClick={handleSave} className="px-6 py-2.5 rounded-xl font-bold bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow-sm">
